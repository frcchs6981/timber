...
public class RobotContainer {
  ...
  new JoystickButton(m_driverController, Button.kR1.value).whileTrue(Commands.runOnce(() -> m_robotElevate.moveElevator(1,1), m_robotElevate));
  new JoystickButton(m_driverController, Button.kL1.value).whileTrue(Commands.runOnce(() -> m_robotElevate.moveElevator(-1,0), m_robotElevate));
  new JoystickButton(m_driverController, Button.kR1.value).whileFalse(Commands.runOnce(() -> m_robotElevate.stopElevator(1), m_robotElevate));
  new JoystickButton(m_driverController, Button.kL1.value).whileFalse(Commands.runOnce(() -> m_robotElevate.stopElevator(0), m_robotElevate));
...
//new JoystickButton(m_driverController, Button.kCross.value).whileTrue(new RunCommand(() -> m_robotElevate.DownToggle(), m_robotElevate));
  ...
  public Command getAutonomousCommand() {
    ...
    Trajectory test1 = TrajectoryGenerator.generateTrajectory(
        // Start at the origin facing the +X direction --- just go straight
    ...
    m_robotDrive.resetOdometry(test1.getInitialPose());


    // Run path following command, then stop at the end.
    return swerveControllerCommand.andThen(() -> m_robotDrive.drive(0, 0, 0, DriveConstants.kIsFieldRelative));
  }
}
